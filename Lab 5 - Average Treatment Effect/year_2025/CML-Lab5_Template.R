###############################################
### 	LAB 3: The Average Treatment Effect    ###
###############################################
setwd()
...
fit <- glm(formula =, family = , data = ) #complete

dat0 <- dat; dat0$A <- 0
dat1 <- dat; dat1$A <- 1

Q0 <- predict(fit, newdata = , type = ) #complete
Q1 <- predict(fit, newdata = , type = ) #complete

ate_gcomp_byhand <- mean(); ate_gcomp_byhand #complete
...
ate_gcomp_se <- sqrt(fit.std$vcov[] + fit.std$vcov[] - 2* fit.std$vcov[]); ate_gcomp_se #complete



# G-Computation Parametric with functions (b - alternative)

muhat_par <- estimate_mu_par(Y = ,
                             A = ,
                             X = ,
                             family = ) #complete
...
Q1 <- predict(Q.sl, newdata = )$pred #complete
Q0 <- predict(Q.sl, newdata = )$pred #complete
...
muhat_est_nonpar <- estimate_mu_nonpar(Y = ,
                                       A = ,
                                       X = ,
                                       sl_lib = ,
                                       nsplits = ,
...
ate_aipw_by_hand <- #complete

influence_curve_aipw <-  #complete

ate_aipw_by_hand_se <-  #complete
...
# Targeting step
Q1_tmle1 <- plogis() #complete
Q0_tmle1 <- plogis() #complete
