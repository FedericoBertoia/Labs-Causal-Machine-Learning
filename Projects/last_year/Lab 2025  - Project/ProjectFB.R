...
theta_hat <- mean((A / mean(A)) * (Y - Q0_tmle))

IC <- (A / mean(A)) * (Y - Q0_tmle - theta_hat) - ((1 - A) * pi / (1 - pi)) * (Y - Q0_tmle) / mean(A)
...
for (k in 1:3){
  ...
  model.pseudo <- SuperLearner(Y = Ci.trainAB, X = L2[c(trainA, trainB), ], SL.library = SL.library, family = gaussian())
